...
expand_environ_var(char *arg)
{
	...
	if ((arg[0] == '$') && (arg_len > 1)) {
		...
		char *envariable_value = getenv(arg + 1);
		if (envariable_value == NULL) {
			arg[0] = END_STRING;
			return arg;
		}

		size_t value_len = strlen(envariable_value);
		if (value_len >= arg_len) {
			arg = realloc(arg, sizeof(char) * (value_len + 1));
			if (!arg) {
				perror("Error de realocación de memoria");
				_exit(EXIT_FAILURE);
			}
		}

		return strcpy(arg, envariable_value);
	}
